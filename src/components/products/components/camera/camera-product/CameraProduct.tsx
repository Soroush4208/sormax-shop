...
function CameraProduct({
  ...
  const [cameraProducts, setCameraProducts] = useState<ProductsType[]>([]);
  const [showSkeleton, setShowSkeleton] = useState(true);
  const [page, setPage] = useState(1);
  const productsPerPage = 12;
  const indexOfLastProduct = page * productsPerPage;
  const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
  const currentProducts = cameraProducts.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
  ...
  useEffect(() => {
    if (products) {
      ...
      setCameraProducts(filteredProducts);
    }
  }, [products]);
        ...
        >
          {cameraProducts.length > 0 && (
            <Stack sx={{ fontSize: "28px", fontWeight: "300" }} spacing={2}>
              <Pagination
                count={Math.ceil(cameraProducts.length / productsPerPage)}
                page={page}
