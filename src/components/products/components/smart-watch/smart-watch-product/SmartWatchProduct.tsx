...
function SmartWatchProduct({
  ...
  const [smartWatchProducts, setSmartWatchProducts] = useState<ProductsType[]>(
    []
  );
  ...
  useEffect(() => {
    if (products) {
      ...
      setSmartWatchProducts(filteredProducts);
    }
  }, [products]);
        ...
        >
          {smartWatchProducts.length > 0 && (
            <Stack sx={{ fontSize: "28px", fontWeight: "300" }} spacing={2}>
              <Pagination
                count={Math.ceil(smartWatchProducts.length / productsPerPage)}
                page={page}
