...
function HeadphoneProduct({
  ...
  const [headphoneProducts, setHeadphoneProducts] = useState<ProductsType[]>(
    []
  );
  ...
  const currentProducts = headphoneProducts.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
        ...
        >
          {headphoneProducts.length > 0 && (
            <Stack sx={{ fontSize: "28px", fontWeight: "300" }} spacing={2}>
              <Pagination
                count={Math.ceil(headphoneProducts.length / productsPerPage)}
                page={page}
