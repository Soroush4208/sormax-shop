...
function MonitorProduct({
  ...
  const [monitorProducts, setMonitorProducts] = useState<ProductsType[]>([]);
  const [showSkeleton, setShowSkeleton] = useState(true);
  const [page, setPage] = useState(1);
  const productsPerPage = 12;
  const indexOfLastProduct = page * productsPerPage;
  const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
  const currentProducts = monitorProducts.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
  ...
  useEffect(() => {
    if (products) {
      ...
      setMonitorProducts(filteredProducts);
    }
  }, [products]);
        ...
        >
          {monitorProducts.length > 0 && (
            <Stack sx={{ fontSize: "28px", fontWeight: "300" }} spacing={2}>
              <Pagination
                count={Math.ceil(monitorProducts.length / productsPerPage)}
                page={page}
